...
export function startApi(): http.Server {
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (p === "/" || p === "/api") return send(res, 200, { name: "tapflow-indexer", endpoints: ["/api/health", "/api/stats", "/api/leaderboard", "/api/leader/:address", "/api/feed", "/api/follows/:address", "/api/og"] });
      if (p === "/api/health") {
        const s = stats();
