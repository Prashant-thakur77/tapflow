...
## 19. Redeeming from a contract works, but the inputs it needs are scattered

**What we built.** MirrorVault (v3) is the pool caller for every mirrored order, so the outcome tokens land on the vault. To hand winnings back to followers the vault calls `BinaryMarketsModule.redeem(operatorId, venueId, marketId, outcomeIdx, amount)` after a one-time `outcomeToken.setOperator(module, true)`. This works first time on Shannon (block 482411335 for the mirror; `redeem` verified in Foundry against a mock of the same surface).
