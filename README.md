...
| **F3** | Copy-trading contracts (reactivity) | ✅ **deployed + proven on Shannon (v3)** | same-block mirrors in [block 482312219](https://shannon-explorer.somnia.network/block/482312219) (v1) … [block 482411335](https://shannon-explorer.somnia.network/block/482411335) (v3); subscription `16791404`; followers redeem mirrored winnings; `forge test` 15/15 |
...
Everything above runs against real Shannon transactions from wallet `0x6798…F228` (leader) and `0x1AD9…2015` (follower). Only the RiskGuard subscription is still pending: it needs another 32 STT, which is one more faucet claim away. The indexer has paired **10 of 10** reactive mirrors with their broadcast in the same block across v1, v2 and v3.
...
- **Deployed on Shannon** (`contracts/deployments.json`):
  ...
  | MirrorVault (v3) | [`0x1a9c46409a34511e05E0552618b81C818D5f0C12`](https://shannon-explorer.somnia.network/address/0x1a9c46409a34511e05E0552618b81C818D5f0C12) — tracks each follower's shares per window and **redeems settled winnings through the BinaryMarketsModule** (`redeem` / `redeemMany`), crediting the follower's deposit |
  | CopyHandler (v3) | [`0x515d5186314Ac4956F08D11B5Aab55Cd5169920d`](https://shannon-explorer.somnia.network/address/0x515d5186314Ac4956F08D11B5Aab55Cd5169920d) — subscription **16791404** on `Router.PositionOpened`, funded with 33 STT |
  | RiskGuard (v3) | [`0xA8a1FD962f1470c7b19D55fa740E00463a9B4b99`](https://shannon-explorer.somnia.network/address/0xA8a1FD962f1470c7b19D55fa740E00463a9B4b99) — deployed + wired; subscription pending the next 32 STT faucet claim |
