// Self-contained DreamDEX Event Contracts helpers for the bot.
//
// A node-friendly mirror of the web app's src/lib/ec (which reads Vite env).
// Same rules: gate on on-chain status, size a stake over the YES/NO book,
// send an IOC, check the receipt, cap expiry at the market's own.
//
...
export const cadences = (windows: TapWindow[], asset: Asset) =>
  [...new Set(windows.filter((w) => w.asset === asset).map((w) => w.intervalSec))].sort((a, b) => a - b);
...
export function topOf(book: BinaryOrderBook): TopOfBook {
...
/** Opening price the window settles against (integer in the oracle's scale, 2 dp today). */
export async function openingPrice(client: SomniaMarketsClient, w: TapWindow, spot: number | null): Promise<number | null> {
  try {
    const r = await client.getOpeningPrices([w.marketId]);
    const hit = Object.entries(r).find(([k]) => k.toLowerCase() === w.marketId.toLowerCase());
    const raw = hit?.[1] ? Number(hit[1]) : null;
    if (!raw || raw <= 0) return null;
    if (!spot) return raw / 100;
    const scales = [1e2, 1e6, 1e8, 1e18, 1];
    const s = scales.reduce((best, x) => (Math.abs(raw / x - spot) < Math.abs(raw / best - spot) ? x : best), 1e2);
    return raw / s;
  } catch {
    return null;
  }
...
export const fmtProb = (p: number | null | undefined) => (p === null || p === undefined || !Number.isFinite(p) ? "—" : `${Math.round(p * 100)}%`);
export const fmtMultiple = (avg: number) => (avg > 0 ? `${(1 / avg).toFixed(2)}×` : "—");
...
export const parseCadence = (s: string): number | null => {
  const m = /^(\d+)\s*(m|h|min|hr)?$/i.exec(s.trim());
  if (!m) return null;
  const n = Number(m[1]);
  return /^h/i.test(m[2] ?? "m") ? n * 3600 : n * 60;
};
export const fmtPx = (n: number) => `$${n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
